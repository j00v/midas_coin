    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.001 MDS per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeMidasAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nZeromintPercentage)) {
        ...
        if (pwalletMain->SelectCoinsDark(nValueMin, 999.99 * COIN, vCoins, nValueIn, -2, 0)) {
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100MDS+1 ( bit on if present )
    // bit 1 - 10MDS+1
    // bit 2 - 1MDS+1
    // bit 3 - .1MDS+1
    // bit 3 - non-denom
