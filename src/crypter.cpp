...
{
    {
        ...
        if (CWalletDB(pwalletMain->strWalletFile).ReadCurrentSeedHash(hashSeed)) {
            ...
            if (!GetDeterministicSeed(hashSeed, nSeed)) {
                return error("Failed to read Midas seed from DB. Wallet is probably corrupt.");
            }
            pwalletMain->zwalletMain->SetMasterSeed(nSeed, false);
        } else {
            // First time this wallet has been unlocked with dMDS
    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteMDSSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteMDSSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save mdsseed to wallet";
    }
                //the use case for this is no password set seed, mint dMDS,
...
bool CCryptoKeyStore::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadMDSSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadMDSSeed(hashSeed, vchSeed)) {
