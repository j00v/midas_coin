...
enum ZerocoinSpendStatus {
    ...
    MDS_WALLET_LOCKED = 2,                         // Wallet was locked
    MDS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    MDS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    MDS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    MDS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    MDS_TRX_CREATE = 7,                            // Everything related to create the transaction
    MDS_TRX_CHANGE = 8,                            // Everything related to transaction change
    MDS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    MDS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    MDS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    MDS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    MDS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    MDS_SPENT_USED_MDS = 14,                      // Coin has already been spend
    MDS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    MDS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void MdsBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateMDSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fmdsAuto) const;
    ...
    */
    ...
    CMDSWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CMDSTracker> mdsTracker;
    ...
    void setZWallet(CMDSWallet* zwallet)
    {
        zwalletMain = zwallet;
        mdsTracker = std::unique_ptr<CMDSTracker>(new CMDSTracker(strWalletFile));
    }

    CMDSWallet* getZWallet() { return zwalletMain; }
    ...
    void setMdsAutoBackups(bool fEnabled)
    {
