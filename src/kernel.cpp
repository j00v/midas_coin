...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZMdsStake(spend));
    } else {
        ...
        CMdsStake* mdsInput = new CMdsStake();
        mdsInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(mdsInput);
    }
