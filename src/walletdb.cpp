...
bool CWalletDB::ArchiveDeterministicOrphan(const CDeterministicMint& dMint)
{
    ...
    if (!Erase(make_pair(string("dmds"), dMint.GetPubcoinHash())))
...
bool CWalletDB::WriteMDSSeed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseMDSSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteMDSSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseMDSSeed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadMDSSeed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadMDSSeed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteMDSCount(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadMDSCount(uint32_t& nCount)
{
...
std::list<CDeterministicMint> CWalletDB::ListDeterministicMints()
{
    ...
    for (;;)
    {
        ...
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << make_pair(string("dmds"), uint256(0));
        CDataStream ssValue(SER_DISK, CLIENT_VERSION);
        int ret = ReadAtCursor(pcursor, ssKey, ssValue, fFlags);
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND)
