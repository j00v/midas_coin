...
CMDSTracker::CMDSTracker(std::string strWalletFile)
{
...
CMDSTracker::~CMDSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CMDSTracker::Init()
{
...
bool CMDSTracker::Archive(CMintMeta& meta)
{
...
bool CMDSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CMDSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CMDSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CMDSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CMDSTracker::GetSerialHashes()
{
...
CAmount CMDSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CMDSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CMDSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CMDSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CMDSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CMDSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CMDSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CMDSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CMDSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CMDSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CMDSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CMDSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CMDSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CMDSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CMDSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CMDSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CMDSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dmds from DB\n", __func__, listDeterministicDB.size());
    }
