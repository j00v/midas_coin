...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MDS and MDS money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MDS/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MDS/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MDS/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-mdsstake=<n>", strprintf(_("Enable or disable staking functionality for MDS inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-mdsstake=<n>", strprintf(_("Enable or disable staking functionality for MDS inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupmds=<n>", strprintf(_("Enable automatic wallet backups triggered after each MDS minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-mdsbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic MDS backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizemidasamount=<n>     " + strprintf(_("Keep N MDS anonymized (default: %u)"), 0) + "\n";
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        uiInterface.InitMessage(_("Syncing MDS wallet..."));
