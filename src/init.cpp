...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "midas.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "midasd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the Midas and Midas money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 44444, 44441));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in Midas/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in Midas/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, midas, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in Midas/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Midas specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-mdsstake=<n>", strprintf(_("Enable or disable staking functionality for Midas inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-mdsstake=<n>", strprintf(_("Enable or disable staking functionality for Midas inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:44444"));
    ...
    strUsage += HelpMessageOpt("-backupmds=<n>", strprintf(_("Enable automatic wallet backups triggered after each Midas minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-mdsbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic Midas backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizemidasamount=<n>     " + strprintf(_("Keep N Midas anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 44445, 44448));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Midas Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Midas is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Midas Core is shutting down."));
...
#endif
    // Make sure only a single Midas process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Midas Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //Midas: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Midas specific: zerocoin and spork DB's
                ...
                // Midas: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateMDSMinted();
                        RecalculateMDSSpent();
                    }
                    RecalculateMDSSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // Midas: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Midas Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Midas Core to complete") << "\n";
        ...
        zwalletMain = new CMDSWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize MDSWallet
        uiInterface.InitMessage(_("Syncing Midas wallet..."));

        bool fEnableMdsBackups = GetBoolArg("-backupmds", true);
        pwalletMain->setMdsAutoBackups(fEnableMdsBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->mdsTracker->Init();
...
//    }
//
//    nAnonymizeMidasAmount = GetArg("-anonymizemidasamount", 0);
//    if (nAnonymizeMidasAmount > 999999) nAnonymizeMidasAmount = 999999;
//    if (nAnonymizeMidasAmount < 2) nAnonymizeMidasAmount = 2;
