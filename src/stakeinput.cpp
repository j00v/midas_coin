...
CZMdsStake::CZMdsStake(const libzerocoin::CoinSpend& spend)
{
...
int CZMdsStake::GetChecksumHeightFromMint()
{
...
int CZMdsStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZMdsStake::GetChecksum()
{
    return nChecksum;
}
...
CAmount CZMdsStake::GetValue()
{
    return denom * COIN;
}

//Use the first accumulator checkpoint that occurs 60 minutes after the block being staked from
bool CZMdsStake::GetModifier(uint64_t& nStakeModifier)
{
...
bool CZMdsStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZMdsStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZMdsStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CMDSTracker* mdsTracker = pwallet->mdsTracker.get();
    CMintMeta meta;
    if (!mdsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    mdsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
bool CMdsStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CMdsStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CMdsStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CMdsStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CMdsStake::GetModifier(uint64_t& nStakeModifier)
{
...
CBlockIndex* CMdsStake::GetIndexFrom()
{
    uint256 hashBlock = 0;
    CTransaction tx;
