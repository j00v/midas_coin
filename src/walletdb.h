...
public:
    ...
    bool WriteMDSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadMDSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadMDSSeed_deprecated(uint256& seed);
    bool EraseMDSSeed();
    bool EraseMDSSeed_deprecated();

    bool WriteMDSCount(const uint32_t& nCount);
    bool ReadMDSCount(uint32_t& nCount);
