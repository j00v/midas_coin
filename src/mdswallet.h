...
#ifndef Midas_MDSWALLET_H
#define Midas_MDSWALLET_H
...
public:
    CMDSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicMDS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToMDS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Midas_MDSWALLET_H
