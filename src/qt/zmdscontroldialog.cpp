...
#include "ui_zmdscontroldialog.h"
...
std::set<std::string> ZMdsControlDialog::setSelectedMints;
std::set<CMintMeta> ZMdsControlDialog::setMints;

ZMdsControlDialog::ZMdsControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMdsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMdsControlDialog::~ZMdsControlDialog()
{
    delete ui;
}

void ZMdsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMdsControlDialog::updateList()
{
...
void ZMdsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMdsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelMds_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMdsControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMdsControlDialog::GetSelectedMints()
{
...
void ZMdsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
