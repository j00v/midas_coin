...
bool parseBitcoinURI(const QUrl& uri, SendCoinsRecipient* out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::MDS, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MDS, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
