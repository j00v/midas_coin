                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    ui->labelMDSSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->MDSPayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintMDS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_MDS, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonSpendMDS_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_MDS, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
...
void PrivacyDialog::on_pushButtonZMdsControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZMdsControlDialog* MdsControl = new ZMdsControlDialog(this);
    MdsControl->setModel(walletModel);
    MdsControl->exec();
}

void PrivacyDialog::setZMdsControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelMdsSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendMDS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Midas Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->MDSPayAmount->text().toDouble();
    ...
    if (!ZMdsControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZMdsControlDialog::GetSelectedMints();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Mds, ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->mdsTracker->GetMints(false);
...
{
    ui->labelMDSSyncStatus->setVisible(fShow);
}
