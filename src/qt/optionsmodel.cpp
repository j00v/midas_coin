...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MDS);
    ...
    if (!settings.contains("nAnonymizeMidasAmount"))
        settings.setValue("nAnonymizeMidasAmount", 1000);

    nAnonymizeMidasAmount = settings.value("nAnonymizeMidasAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMidasAmount"))
        SoftSetArg("-anonymizemidasamount", settings.value("nAnonymizeMidasAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMidasAmount:
            return QVariant(nAnonymizeMidasAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMidasAmount:
            nAnonymizeMidasAmount = value.toInt();
            settings.setValue("nAnonymizeMidasAmount", nAnonymizeMidasAmount);
            emit anonymizeMidasAmountChanged(nAnonymizeMidasAmount);
            break;
        case CoinControlFeatures:
