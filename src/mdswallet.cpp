...
CMDSWallet::CMDSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing mds seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadMDSSeed_deprecated(seed)) {
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of mds wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CMDSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteMDSCount(nCountLastUsed);
    else if (!walletdb.ReadMDSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CMDSWallet::Lock()
{
    seedMaster = 0;
}

void CMDSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CMDSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToMDS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CMDSWallet::LoadMintPoolFromDB()
{
...
void CMDSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CMDSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CMDSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->mdsTracker->HasPubcoinHash(pMint.first)) {
...
bool CMDSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToMDS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to mdsTracker which also adds to database
    pwalletMain->mdsTracker->Add(dMint, true);
...
void CMDSWallet::SeedToMDS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CMDSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CMDSWallet::GenerateDeterministicMDS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CMDSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CMDSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
