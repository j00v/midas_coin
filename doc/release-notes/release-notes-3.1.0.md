Midas Core version *3.1.0* is now available from:  <https://github.com/midas-project/midas/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/midas-project/midas/issues>


Mandatory Update
==============

Midas Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
The visual layout of the Midas Qt wallet has undergone a near-complete overhaul.
...
### Summary
 ...
 The Midas wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the midas.conf file, found in the Midas data directory.
- backuppath = <directory / full path>
- mdsbackuppath = <directory / full path>
...
Midas Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The Midas client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running midasd without having to do any manual configuration.
...
Midas's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which Midas used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to Midas using the bug bounty program. The modulus is now correctly imported and Midas's accumulators have been changed to use the new proper modulus.
...
### GUI
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "Mds Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault midas-cli getinfo while loading block index (Mrs-X)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/midas-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
