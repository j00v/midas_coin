...
Midas Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for MDS spends, automint calculation adjustments, and other various updates/fixes.

MDS spending requires this update.
...
MDS Automint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in MDS mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

MDS Spending Fix
---------------------
The size of MDS spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

MDS Transaction Recovery
---------------------
Due to the aforementioned issue with MDS spending, users may find that their attempted spends are now conflicted and MDS balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
