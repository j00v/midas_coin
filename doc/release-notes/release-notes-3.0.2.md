Midas Core version 3.0.2 is now available from:
...
Midas Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for Midas spends, automint calculation adjustments, and other various updates/fixes.

Midas spending requires this update.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each Midas mint operation (Midas spends which re-mint their change are also included in this). This functionality is controlled by the `-backupmds` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupmds=0` at the command-line when starting the client, or add `backupmds=0` to their `midas.conf` file.

Midas Automint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in Midas mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

Midas Spending Fix
---------------------
The size of Midas spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

Midas Transaction Recovery
---------------------
Due to the aforementioned issue with Midas spending, users may find that their attempted spends are now conflicted and Midas balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the Midas address as it's first parameter and the passphrase as it's second parameter.
...
- #292 `feadab4` Additional checks for double spending of Mds serials. (presstab)

### Wallet
- #271 `5e9a086` [Wallet] Remove unused member wallet in UnlockContext inner class (Jon Spock)
- #279 `e734010` Add -backupmds startup flag. (presstab)
- #280 `fdc182d` [Wallet] Fix Mds spending errors. (presstab)
- #282 `310f216` [Wallet] Count pending Mds balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] Mds control quantity/amount fixes (rejectedpromise)
